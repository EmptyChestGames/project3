...
function PlayState:init()
    ...
    self.timer = 60
    self.clearTimer = 3
    -- ensure that we have a board with that starts with possible matches.
...
function PlayState:ensureBoard()
    local ensure = self.board:testBoard()
    if ensure == 0 then
    ...
    if self.canInput then
        ...
        if love.keyboard.wasPressed('enter') or love.keyboard.wasPressed('return') then
            ...
            elseif math.abs(self.highlightedTile.gridX - x) + math.abs(self.highlightedTile.gridY - y) > 1 then
                ...
                local test = self:testMatches(self.highlightedTile, newTile)
                if test == true then
                    Timer.tween(0.1, {
    ...
    if matches then
        ...
        if DESTRUCTION == false then
            for k, match in pairs(matches) do
                self.score = self.score + #match * VARIETYBONUS * 50
                self.timer = self.timer + #match
            end
        end
        if DESTRUCTION == true then
            self.score = self.score + 400 * VARIETYBONUS
...
function PlayState:testBoard()
    local reset = self.board:testBoard()
    if reset == 0 then
        self.board:initializeTiles()
    end
end
